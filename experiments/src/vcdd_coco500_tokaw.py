"""
vcdd_coco500_tokaw.py - Offline VCD Distillation with Token-Level Agreement Weighting
...
Token-Level Agreement Weighting (升級自 sample-level scalar AW):
  原本：整個 answer sequence 共用一個 scalar correction_weight
  現在：每個 token 有自己的 w_i = clamp(KL_i * aw_scale, max=aw_max_weight)

  KL_i = KL(p_clean_i ∥ p_vcd_i)  — 第 i 個 token 位置的分佈差異
  → 高 KL token（物體名詞、方位詞等）獲得高學習權重
  → 低 KL token（語氣詞、標點等）獲得接近 0 的權重
  → 信噪比大幅提升，避免背景詞稀釋真正的 VCD 校正信號
    ...
    for epoch in range(args.epochs):
        ...
        for step, line in enumerate(tqdm(questions, desc=f"Epoch {epoch+1}")):
            ...
            if not args.sft_only:
                ...
                # Token-Level AW: 每個 token 有自己的 correction weight
                correction_weight = torch.clamp(
                    kl_per_token * args.aw_scale, max=args.aw_max_weight
                )  # (A,) tensor
                raw_cw = kl_per_token.mean().item()  # scalar，僅用於 logging
            else:
                soft_labels       = None
                correction_weight = 1.0
                raw_cw            = 1.0
